...
global.money += 0.8 * round((((global.cashwavereward) * (global.cashflow))) * (1 + global.cashinflate * 0.1));
global.money += 0.8 * (global.wealthiness * 20) - 1 + (1 * global.wave);
...
with (BloonSpawn) {
var inst;
inst = instance_create(x, y, Super_Duper_Bloon);
with (inst)
   {
   bloonmaxlayer = 100;
   bloonlayer = 100;
