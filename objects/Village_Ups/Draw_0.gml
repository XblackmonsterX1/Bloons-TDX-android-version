image_index = MVup

if global.MVL = 1
if MVup = 3
{
image_index = 11;
}

if global.MVM = 1
if MVup = 6
{
image_index = 11;
}

if global.MVR = 1
if MVup = 9
{
image_index = 11;
}

if global.MVL < 3
if MVup = 4
{
image_index = 12;
}

if global.MVM < 3
if MVup = 7
{
image_index = 12;
}

if global.MVR < 3
if MVup = 10
{
image_index = 12;
}

if global.MVR = 11
if MVup = 14
{
image_index = 11;
}

if global.MVR < 13
if MVup = 15
{
image_index = 12;
}

draw_sprite(Monkey_Village_Panel_Spr, image_index, x, y);
if image_index==0
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_0"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_0_desc"))	
}
if image_index==1
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_1"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_1_desc"))	
}
if image_index==2
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_L1"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_L1_desc"))
}
if image_index==3
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_L2"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_L2_desc"))	
}
if image_index==4
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_L3"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_L3_desc"))	
}
if image_index==5
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_M1"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_M1_desc"))	
}
if image_index==6
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_M2"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_M2_desc"))	
}
if image_index==7
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_M3"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_M3_desc"))	
}
if image_index==8
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_R1"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_R1_desc"))	
}
if image_index==9
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_R2"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_R2_desc"))	
}
if image_index==10
{
draw_upg_name(x+85,y+14,get_txt(global.language,"villa_R3"))
draw_upg_desc(x+92,y+35,get_txt(global.language,"villa_R3_desc"))	
}
action_font(font0, 0);
action_color(0);
if global.MVL = 1
if MVup = 3
{
draw_text(x+120, y+40, string_hash_to_newline("x4 "));
}

if global.MVM = 1
if MVup = 6
{
draw_text(x+120, y+40, string_hash_to_newline("x5 "));
}

if global.MVR = 1
if MVup = 9
{
draw_text(x+120, y+40, string_hash_to_newline("x5 "));
}

if global.MVL < 3
if MVup = 4
{
draw_text(x+120, y+40, string_hash_to_newline("x11 "));
}

if global.MVM < 3
if MVup = 7
{
draw_text(x+120, y+40, string_hash_to_newline("x11 "));
}

if global.MVR < 3
if MVup = 10
{
draw_text(x+120, y+40, string_hash_to_newline("x11 "));
}

if global.MVR = 11
if MVup = 14
{
draw_text(x+120, y+40, string_hash_to_newline("x15 "));
}

if global.MVR < 13
if MVup = 15
{
draw_text(x+120, y+40, string_hash_to_newline("x25 "));
}


