with(Bloon){
	if object_index != Bounty
	with instance_create(x,y,Emoji_Face_Obj)
	{
...
if __b__
{
action_timeline_set(LUL_3_Star_30, 0, 0, 0);
}
...
with (BloonSpawn) {
var inst;
inst = instance_create(x, y, HTA_Terrible);
with (inst)
   {
   bloonmaxlayer = 100;
   bloonlayer = 100;
...
with (BloonSpawn) {
var inst;
inst = instance_create(x, y, Emoji_God);
with (inst)
   {
   bloonmaxlayer = 100;
   bloonlayer = 100;
