function scr_Bloon_Paths() {
	...
	if global.track != 12 and global.track != 15 and global.track != 20 and global.track != 21 and global.track != 22 and global.track != 28 and global.track != 30 and global.track != 33 and global.track != 34
	with BloonSpawn
	{
	other.path = path
	if path = 1
	...
	if global.track = 12 || global.track = 22 || global.track = 28
	with BloonSpawn
	{
	other.path = path
	if path < 3
	...
	if global.track = 15 || global.track = 20 || global.track = 30
	with BloonSpawn
	{
	other.path = path
	if path < 2
	...
	{
	if path = 0
	path_start(Graveyard_Path_A, 0, 0, 1);
	move = path_index
	if path = 1
	path_start(Graveyard_Path_B, 0, 0, 1);
	move = path_index
	if path = 2
	path_start(Graveyard_Path_C, 0, 0, 1);
	move = path_index
	}
	if global.track = 21
	{
	...
	{
	if path = 0
	path_start(Lunar_Temple_Path_A, 0, 0, 1);
	move = path_index
	if path = 1
	path_start(Lunar_Temple_Path_B, 0, 0, 1);
	move = path_index
	}
	if global.track = 26
	{
	path_start(Birthday_Party_Path, 0, 0, 1);
	move = path_index
	}
	...
	{
	if path = 0
	path_start(Rubber_Rug_Path_A, 0, 0, 1);
	move = path_index
	if path = 1
	path_start(Rubber_Rug_Path_B, 0, 0, 1);
	move = path_index
	}
	...
	{
	if path = 0
	path_start(Frozen_Lake_Path_A, 0, 0, 1);
	move = path_index
	if path = 1
	path_start(Frozen_Lake_Path_B, 0, 0, 1);
	move = path_index
	if path = 2
	path_start(Frozen_Lake_Path_C, 0, 0, 1);
	move = path_index
	if path = 3
	path_start(Frozen_Lake_Path_D, 0, 0, 1);
	move = path_index
	}
	...
	{
	if path = 0
	path_start(Sky_Battles_Path_A, 0, 0, 1);
	move = path_index
	if path = 1
	path_start(Sky_Battles_Path_B, 0, 0, 1);
	move = path_index
	}
	...
	{
	if path = 0
	path_start(Lava_Stream_Path_A, 0, 0, 1);
	move = path_index
	if path = 1
	path_start(Lava_Stream_Path_B, 0, 0, 1);
	move = path_index
	if path = 2
	path_start(Lava_Stream_Path_C, 0, 0, 1);
	move = path_index
	}
	...
	{
	if path = 0
	path_start(Ravine_River_Path_A, 0, 0, 1);
	move = path_index
	if path = 1
	path_start(Ravine_River_Path_B, 0, 0, 1);
	move = path_index
	}

	if global.track = 32
	{
	path_start(Peaceful_Bridge_Path, 0, 0, 1);
	move = path_index
	}
	...
	{
	if path = 0
	path_start(Lawless_Badlands_Path_A, 0, 0, 1);
	move = path_index
	if path = 1
	path_start(Lawless_Badlands_Path_B, 0, 0, 1);
	move = path_index
	if path = 2
	path_start(Lawless_Badlands_Path_C, 0, 0, 1);
	move = path_index
	if path = 3
	path_start(Lawless_Badlands_Path_D, 0, 0, 1);
	move = path_index
	if path = 4
	path_start(Lawless_Badlands_Path_E, 0, 0, 1);
	move = path_index
	}
	...
	{
	if path = 0
	path_start(Lost_Pools_Path_A, 0, 0, 1);
	if path = 1
	path_start(Lost_Pools_Path_B, 0, 0, 1);
	if path = 2
	path_start(Lost_Pools_Path_C, 0, 0, 1);
	if path = 3
	path_start(Lost_Pools_Path_D, 0, 0, 1);
	if path = 4
	path_start(Lost_Pools_Path_E, 0, 0, 1);
	if path = 5
	path_start(Lost_Pools_Path_F, 0, 0, 1);
	if path = 6
	path_start(Lost_Pools_Path_G, 0, 0, 1);
	if path = 7
	path_start(Lost_Pools_Path_H, 0, 0, 1);
	if path = 8
	path_start(Lost_Pools_Path_I, 0, 0, 1);
	move = path_index
	}
